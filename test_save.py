...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            torch.save(output, 'visual/outputpt/' + str(number)  + '.pt')

            subcls = subcls[0].cpu().numpy()[0]

            input2 = input.detach().cpu().numpy()
            input2 = input2.squeeze(0)
            input2 = np.transpose(input2,(1,2,0))
            # input2 = input2 * 255

            for k in range(3):
                input2[:, :, k] = input2[:, :, k] * std[k]+ mean[k]
            input2 = cv2.cvtColor(input2, cv2.COLOR_BGR2RGB)
            # input2 = np.transpose(input2,(1,2,0))
            input2 = input2 * 255
            
            # torch.save(target, 'visual/targetpt/' + str(path).split('/')[5][:-7] + '.pt')
            ...
            target_ori = target2
            target2 = target2
            cv2.imwrite('visual/query/'  + str(number) + '.jpg', input2)
            cv2.imwrite('visual/output/' + str(number) + '.jpg', output2)
            cv2.imwrite('visual/label/' + str(number) + '.jpg', target2)
            cv2.imwrite('visual/label2/' + str(number) + '.jpg', target2*255)
            ...
            if ((i + 1) % round((test_num/100)) == 0):
                logger.info('Test: [{}/{}] '
                            ...
                            'Accuracy {accuracy:.4f}.'.format(iter_num* args.batch_size_val, test_num,
                                                            ...
                                                            accuracy=accuracy))
    with open('visual/path.txt', 'w') as file:
