...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            start_time = time.time()
            # _, meta_out, base_out = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls)
            model_time.update(time.time() - start_time)

            output = torch.load('output2pt/' + str(number)  + '.pt')
            # print(output.shape)
            ...
            meta_out, base_out = output, output
